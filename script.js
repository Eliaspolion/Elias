...
/* ── Form submit ─────────────────────────────────── */
qs('#ctBtn').addEventListener('click',function(){
  // Validate form fields
  const inputs = qsa('#contact .ct-field input, #contact .ct-field textarea');
  let valid = true;
  inputs.forEach(i=>{ if(!i.value.trim()) valid = false; });
  // Email check
  const email = qsa('#contact .ct-field input[type=email]')[0];
  if(email && email.value.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value.trim())) valid = false;

  if(!valid){
    this.textContent = 'Please fill the form first';
    this.style.color = 'var(--white)';
    setTimeout(()=>{ this.textContent = 'Send Message'; this.style.color = ''; }, 2500);
    return;
  }

  this.textContent = 'Message received — I\'ll be in touch ✓';
  this.style.color = 'var(--white)';
